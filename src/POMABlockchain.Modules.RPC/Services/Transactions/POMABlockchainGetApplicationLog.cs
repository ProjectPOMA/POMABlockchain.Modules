...
namespace POMABlockchain.Modules.RPC.Services.Transactions
{
    ...
    ///     Contract Log
    ///     <note type="important">
    ///     You need to run the command dotnet POMABlockchain-cli.dll --log to enable logging before invoking this method. The complete contract logs are stored under the ApplicationLogs directory.
    ///     </note>
    /// 
    ///     Example
    ///     Request
    ...
    /// }
    /// </Summary>
    public class POMABlockchainGetApplicationLog : RpcRequestResponseHandler<DTOs.ApplicationLog>
    {
        public POMABlockchainGetApplicationLog(IClient client) : base(client, ApiMethods.getapplicationlog.ToString())
        {
        }
