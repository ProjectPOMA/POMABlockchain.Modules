...
namespace POMABlockchain.Modules.RPC.Services.Account
{
    ...
    /// 
    ///     Returns
    ///     Script_hash: Contract scipt hash; All accounts in POMABlockchain are contract accounts
    ///     Frozen: Determine if the account is frozen
    ///     Votes: Query the amount of POMABlockchain on that address used to vote
    ...
    /// }
    /// </summary>
    public class POMABlockchainGetAccountState : RpcRequestResponseHandler<AccountState>
    {
        public POMABlockchainGetAccountState(IClient client) : base(client, ApiMethods.getaccountstate.ToString())
        {
        }
